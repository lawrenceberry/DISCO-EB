...
def test_cmb_vs_camb(cosmology_name, camb_benchmarks, num_regression, benchmark):
    ...
    To update the accuracy baseline after intentional improvements:
        pytest tests/test_cmb.py::test_cmb_vs_camb --force-regen

    To update the timing baseline after intentional improvements:
        pytest tests/test_cmb.py::test_cmb_vs_camb --benchmark-autosave
    """
    import jax
    jax.config.update("jax_enable_x64", True)
    jax.config.update('jax_platform_name', 'gpu')
    ...
    # Verify output
    assert Cell_disco.shape == (2501,)  # ellmax + 1
