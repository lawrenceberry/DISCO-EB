...
Ported from the DISCO2 prototype and generalized to a parametric dense-core
size. This is the ``custom_lu_solver`` consumed by
    ...
    def assemble_lu_local_factory(self, jac_device, n_vars, lu_dtype):
        ...
        def assemble_lu(y_local, t, p_row, lu_buf, dtgamma_inv):
            ...
            for b in range(n_blocks):
                ...
                v_d = values[kb + last][kb + last]
                m_d = dtgamma_inv - v_d
                inv_d = 1.0 / m_d
                lu_buf[invd_off + b * tl + last] = lu_dtype(inv_d)

                v_l = values[kb + last][kb + last - 1]
                m_l = -v_l
                ...
                for idx in range(last - 1, -1, -1):
                    ...
                    if idx > 0:
                        ...
                        # Coupling of the block's lowest multipole to the dense core.
